...
fsp_err_t wrSensorReg16_8(i2c_master_ctrl_t * p_api_ctrl, uint16_t regID, uint8_t regDat)
{
    ...
    err = R_SCI_B_I2C_Write(p_api_ctrl, data, 3, false);
    if (FSP_SUCCESS == err)
    {
        err = i2c_cam_cb_wait();
    }
...
fsp_err_t rdSensorReg16_8(i2c_master_ctrl_t * p_api_ctrl, uint16_t regID, uint8_t* regDat)
{
    ...
    err = R_SCI_B_I2C_Write(p_api_ctrl, data, 2, true);
    if (FSP_SUCCESS == err)
    {
        err = i2c_cam_cb_wait();
        if (FSP_SUCCESS == err)
        {
            err = R_SCI_B_I2C_Read(p_api_ctrl, regDat, 1, false);
            if (FSP_SUCCESS == err)
            {
                err = i2c_cam_cb_wait();
            }
...
fsp_err_t rdSensorReg16_Multi(i2c_master_ctrl_t * p_api_ctrl, uint16_t regID, uint8_t* regDat, uint32_t len)
{
    ...
    if (FSP_SUCCESS == err)
    {
        ...
        if (FSP_SUCCESS == err)
        {
            err = R_SCI_B_I2C_Read(p_api_ctrl, regDat, len, false);
            if (FSP_SUCCESS == err)
            {
                err = i2c_cam_cb_wait();
            }
